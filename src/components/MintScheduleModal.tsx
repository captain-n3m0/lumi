...
export function MintScheduleModal({
  ...
  const stages = useMemo(() => {
    ...
    return getCollectionMintStages(activeCollection.name, activeCollection.slug);
  }, [activeCollection]);
  ...
  const handleUrlSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (results.length > 0) {
        setActiveCollection(results[0]);
        if (onSelectCollection) onSelectCollection(results[0]);
        setSelectedStage(null);
      }
    } finally {
          ...
          <div className="p-4 sm:p-6 flex flex-col gap-5 max-h-[80vh] overflow-y-auto">
                    ...
                    <div className="p-4 pt-5 flex flex-col gap-3">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex flex-col min-w-0">
                          <h3 className="font-extrabold text-sm sm:text-base text-foreground tracking-tight uppercase">
                            {stage.name}
                          </h3>
                        </div>
            ...
            <div className="mt-2 pt-4 border-t border-border/80 flex flex-col sm:flex-row items-center justify-between gap-3">
              <button
                type="button"
                onClick={() => {
                  if (stages[0]) handleScheduleStage(stages[0]);
                }}
                disabled={success}
                className="w-full sm:w-auto px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-bold text-xs sm:text-sm shadow-md hover:brightness-110 active:scale-98 transition flex items-center justify-center gap-2 cursor-pointer"
              >
                {success ? (
                  <>
                  ...
                  <span>Only accessible to guests & customers</span>
                )}
              </button>
