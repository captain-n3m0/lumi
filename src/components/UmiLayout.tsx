            ...
            <div className="mt-2 border-t border-border px-1 pt-3">
              {wallet.isConnected && wallet.address ? (
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 leading-tight min-w-0">
                    <p className="text-xs font-semibold text-foreground truncate">
                      {wallet.balanceFormatted} {wallet.chainId === 137 ? "POL" : "ETH"}
                    </p>
                  ...
                  <button
                    ...
                    className="text-muted-foreground transition-colors hover:text-rose-400 p-1 rounded-md"
                    title="Disconnect"
                  >
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg bg-primary text-primary-foreground text-xs font-medium hover:opacity-90 transition active:scale-98 cursor-pointer"
                >
                  <Wallet className="size-3.5" />
                  {wallet.isConnecting ? "Connecting..." : "Connect Web3 Wallet"}
                </button>
              )}
            </div>
          </div>
      ...
      {query.trim().length > 0 && (
        <div className="flex flex-col gap-1.5 animate-in fade-in duration-150">
          <div className="px-1 text-xs font-normal text-muted-foreground/80">
            {results.length} collections & contracts found
          </div>
          ...
          {results.length > 0 ? (
              ...
              {results.map((col) => (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-1.5">
                        ...
                        {col.openseaUrl && (
                          <a
                            ...
                            title="View on OpenSea / Explorer"
                          >
                            <ExternalLink className="size-3.5" />
                          </a>
                        )}
                      ...
                      <div className="mt-0.5 flex items-center gap-2 text-xs font-mono text-muted-foreground">
                        <span className="text-primary font-medium uppercase text-[10px]">
                          {col.chain}
                        </span>
                        <span>•</span>
                  ...
                  <button
                    ...
                    className="shrink-0 text-xs bg-primary text-primary-foreground font-medium px-3 py-1.5 rounded-lg opacity-90 group-hover:opacity-100 transition shadow-xs cursor-pointer"
                  >
                    Select
                  </button>
                </div>
            ...
            !loading && (
              <div className="rounded-xl border border-border bg-card p-6 text-center text-sm text-muted-foreground">
                No collections found for "{query}". You can also paste any raw contract address
                (0x...).
              </div>
            )
          )}
